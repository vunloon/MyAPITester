...
const TabTitle = ({ name, isDirty, isActive }: { name: string, isDirty: boolean, isActive: boolean }) => {
  ...
  useEffect(() => {
    ...
    const checkOverflow = () => {
      if (textRef.current) {
        setShouldScroll(textRef.current.scrollWidth > 100);
      }
    };
  ...
  }, [name, isDirty, isActive, shouldScroll]);

  return (
    <div className="max-w-[100px] overflow-hidden flex items-center">
      <span 
        ref={textRef}
        className={`text-xs font-medium ${shouldScroll ? 'inline-block animate-scroll-text whitespace-nowrap' : 'block truncate'}`}
      >
        {name}{isDirty ? '*' : ''}
      </span>
    </div>
...
function App() {
  ...
  const [popupInfo, setPopupInfo] = useState<{ id: string, name: string, top: number, left: number } | null>(null);
  ...
  return (
    <div className="flex h-screen w-screen bg-transparent text-text-secondary font-sans p-4 gap-4 overflow-hidden box-border" style={{ WebkitAppRegion: 'drag' } as React.CSSProperties}>
          ...
          <div className={`flex flex-1 items-center px-4 py-3 gap-2 ${tabBarWrap ? 'flex-wrap overflow-y-auto max-h-48' : 'overflow-x-auto whitespace-nowrap'}`} style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
            {openTabs.map(tab => (
              ...
              >
                ...
                <TabTitle name={tab.name} isDirty={tab.isDirty} isActive={activeTabId === tab.id} />
      ...
      {popupInfo && (
        <div 
          className="fixed z-50 bg-[var(--panel-bg)] text-text-primary text-xs px-3 py-2 rounded-lg shadow-2xl border border-[var(--panel-border)] pointer-events-none break-words max-w-sm"
